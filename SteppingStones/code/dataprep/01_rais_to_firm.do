...
* AUTHOR: LUIS GOMES
* PROGRAM: CLEAN RAIS DATASETS, GENERATE OUTCOMES, GENERATE WORKER PANEL
...
* Clean up temporary variables
drop  rank_composite rank_final
...
//  local i=2009
    ...
    gen new_hire = (year(dtadmissao_stata)==`i')*final_rank // indicates if a spell was started during the current year
...
//     bysort identificad: egen firm_emp_jan = total(emp_jan_dec) // sums by each firmn the number of employees that were active in Jan that remained active in dec
    ...
    * First, ensure that dtnascimento is a string.
    ...
    bysort identificad PIS: gen tag_below_30 = cond(_n == 1 & d_below_30==1 , 1, 0) // marks first spell of an employee active through dec that is younger than 30yo
    ...
    * the earlier generated betw_30_40 from idade might be used.
    bysort identificad PIS: gen tag_30_40 = cond(_n==1 & betw_30_40==1 , 1, 0)
    bysort identificad: egen total_30_40 = total(tag_30_40)
    gen prop_30_40 = total_30_40 / firm_emp
    drop betw_30_40 tag_30_40

    bysort identificad PIS: gen tag_above_40 = cond(_n==1 & above_40==1 , 1, 0)
...
//
//
// save "$interim/rais_firm_`i'.dta", replace
...
compress
save "$analysis/worker_estab_all_years.dta", replace
